% F03_C.m
% Manuscript Fig 3c: functional correspondence by ROI.
% Input tables are read from manuscript_figure_source_data/data/main_text_source_data.xlsx.
clear all
ROI={"Dorsal", "Lateral","Ventral"}

%% read dataTables
Fig3c=read_main_text_source_table("fig3c_points");
Fig3c_noiseCeiling=read_main_text_source_table("fig3c_noise");
%% get all models
% Specify column names and types
disp(Fig3c.Properties.VariableNames);
allmodels =unique(Fig3c.model_type);
allmodels
%%
%allmodels=allmodels([2 3 1 7 6 5 4])% re-order models so plots make sense
allmodels=allmodels([2 3 4 1 8 6 7 5])% re-order models so plots make sense

nmodels=length(allmodels);

Dorsal_i=find(Fig3c.ROIS=="Dorsal");
Dorsal=Fig3c(Dorsal_i,:);
Lateral_i=find(Fig3c.ROIS=="Lateral");
Lateral=Fig3c(Lateral_i,:);
Ventral_i=find(Fig3c.ROIS=="Ventral");
Ventral=Fig3c(Ventral_i,:);
...
for r=1:length(ROI)
    roi_i=find(Fig3c_noiseCeiling.ROI==ROI{r});
    mean_noise(r)=mean(Fig3c_noiseCeiling.result(roi_i));
    sd_noise(r)=std(Fig3c_noiseCeiling.result(roi_i));
end
...
xvals=[1:3 4.5 5.5 6.5 8 9 ];
%XTickLabelString={'MB v1 RN50','MB v2 RN50', 'MB v1 RN18', 'Cat RN18', 'SimCLR RN18', 'TDANN Cat RN18' 'TDANN SimCLR RN18'};
XTickLabelString={'MB v1','MB v2', 'MB v3', 'MB v1', 'Cat', 'SimCLR', 'TDANN Cat' 'TDANN SimCLR'};


%% plot Fig 3c
fig3cFig=figure('Color', [1 1 1], 'Units','normalized','Position',[ 0.1 0.1 .6 .6],'Name','Fig3c')

% RN model
model_names{1} = '50'; %'Det.';
model_names{2} = '50'; %'Act.';
model_names{3} = "50";
model_names{4} = '18'; %'Cat.';
model_names{5} = '18'; %''Det.';
model_names{6} = '18'; %'Act.';
model_names{7} = '18'; %'Cat.';
model_names{8} = '18'; %'Det.';
...
bar(xvals(3),mean_d(3),'FaceColor',teal,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.35)

bar(xvals(4),mean_d(4),'FaceColor',teal,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(5),mean_d(5),'FaceColor',gold,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(6),mean_d(6),'FaceColor',mpurple,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(7),mean_d(7),'FaceColor',gold,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.75)
bar(xvals(8),mean_d(8),'FaceColor',mpurple,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',1)
...
bar(xvals(3),mean_l(3),'FaceColor',teal,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.35)

bar(xvals(4),mean_l(4),'FaceColor',teal,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(5),mean_l(5),'FaceColor',gold,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(6),mean_l(6),'FaceColor',mpurple,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(7),mean_l(7),'FaceColor',gold,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.75)
bar(xvals(8),mean_l(8),'FaceColor',mpurple,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',1)
...
bar(xvals(3),mean_v(3),'FaceColor',teal,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.35)
bar(xvals(4),mean_v(4),'FaceColor',teal,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(5),mean_v(5),'FaceColor',gold,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(6),mean_v(6),'FaceColor',mpurple,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.5)
bar(xvals(7),mean_v(7),'FaceColor',gold,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',0.75)
bar(xvals(8),mean_v(8),'FaceColor',mpurple,'BarWidth',.95,'EdgeColor',"none",'FaceAlpha',1)
...
%% savefigs
exportgraphics(fig3cFig,'Fig3c_checkpoint0_061426.tif','Resolution', 600);
